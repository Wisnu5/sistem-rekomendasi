...
"""## Menggunakan dataset yang diperoleh dari platform Kaggle
pada tahap ini melakukan explorasi data yang terdapat pada platform Kaggle yang nantinya digunakan untuk analisis lebih lanjut dataset apa saja yang akan digunakan dalam membangun sistem rekomendasi
"""
...
"""ada 4 file csv tour_rate, pack, tour_with_id, user"""

# data user
user = pd.read_csv(path + "/user.csv")
user.head()

"""untuk file user terdiri dari kolom user_id, location, dan age"""

user.shape

"""memiliki ukuran 300 baris dan 3 kolom"""

user.isnull().sum()

"""tidak memiliki nilai missing value untuk dataset user"""

# data rating
tour_rate = pd.read_csv(path + "/tourism_rating.csv")
tour_rate.head()

"""terdiri dari 3 kolom untuk tour_rate user_id, place_id, place_ratings"""

tour_rate.shape

"""memiliki 10.000 baris dan 3 kolom"""

tour_rate.isnull().sum()

"""tidak memiliki missing value untuk dataset tour_rate"""

# data pack tour
tour_pack = pd.read_csv(path + "/package_tourism.csv")
tour_pack.head()

"""terdiri dari 7 kolom untuk dataset tour_pack"""

tour_pack.shape

"""memiliki ukuran 100 baris dann 7 kolom"""

# data tour with id
tour_info = pd.read_csv(path + "/tourism_with_id.csv")
tour_info.head()

"""dataset info terdiri dari 13 kolom atau fitur"""
...
"""memiliki missing value pada fitur time_minutes dan unnamed"""

tour_info.duplicated().sum()

"""tidak ada duplikasi data pada dataset tour_info"""

tour_info['Category'].unique()

"""untuk kategori wisatanya ada 6"""
...
# Data Preprocessing
disini melakukan preprocessing untuk kebutuhan sebelum modeling
"""
...
"""Berikut merupakan distribusi 10 tempat wisata yang paling sering dikunjungi
...
Menggunakan sample sebanyak 5 untuk membangun model content based filtering
"""
...
def precision_at_k(recommended_items, relevant_items, k):
    ...
    relevant_recommended = len(set(recommended_at_k) & relevant_items)
...
# Recommendation function returning Place_Id
def rec_with_id(place_name, similarity_data, items, k=10):
...
# Evaluation function
def evaluate_content_based(similarity_data, items_df, k=10):
    precision_scores = []
    recall_scores = []

    sample_items = items_df.sample(min(50, len(items_df)), random_state=42)

    for _, item in sample_items.iterrows():
        query_place_name = item['Place_Name']
        query_city = item['City']
        query_category = item['Category']
        query_place_id = item['Place_Id']

        relevant_items_set = set(items_df[
            (items_df['City'] == query_city) &
            (items_df['Category'] == query_category) &
            (items_df['Place_Id'] != query_place_id)
        ]['Place_Id'].tolist())

        if not relevant_items_set:
            continue

        try:
            recommendations_df = rec_with_id(
                query_place_name,
                similarity_data=similarity_data,
                items=items_df[['Place_Id', 'Place_Name', 'Category', 'Description', 'City']],
                k=k
            )
            recommended_item_ids = recommendations_df['Place_Id'].tolist()

            prec = precision_at_k(recommended_item_ids, relevant_items_set, k)
            rec = recall_at_k(recommended_item_ids, relevant_items_set, k)
    ...
    return {
        f"Average Precision@{k}": avg_precision,
        f"Average Recall@{k}": avg_recall
    }

# Jalankan evaluasi
results = evaluate_content_based(cosine_sim_df, df_baru, k=10)
